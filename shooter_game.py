...
class Enemy(SpaceShip):
    def __init__(self):
        enemy_class = random.randint(1, 11)
        if enemy_class <= 10:
            super().__init__(
                image='e1.png',
...
class Text():
    def __init__(self, text, size, x, y):
        self.font = pygame.font.Font('I-pixel-u.ttf', size)
...
class GameManager():
    def __init__(self):
        self.state = 'play'
        self.score = 0
        self.score_label = Text('0', 30, 30, 30)
        ...
        self.restart_label = Text('Press R to restart', 30, WIDTH // 2, HEIGHT // 2 + 50)
